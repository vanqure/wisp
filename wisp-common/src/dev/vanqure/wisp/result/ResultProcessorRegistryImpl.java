...
final class ResultProcessorRegistryImpl implements ResultProcessorRegistry {
    ...
    ResultProcessorRegistryImpl(final Map<Class<?>, ResultProcessor<?, ?>> processorsByTypeMap) {
        this.processorsByTypeMap = processorsByTypeMap;
    }

    @Override
    public <E extends Event, T> void register(
            final @NotNull Class<T> resultType,
            final @NotNull ResultProcessor<E, T> resultProcessor) {
        processorsByTypeMap.put(resultType, resultProcessor);
    }

    private boolean isAssignableFrom(final @NotNull Class<?> type, final @NotNull Class<?> otherType) {
        return type.isAssignableFrom(otherType) || otherType.isAssignableFrom(type);
    }

    @Override
    public <E extends Event, T> void process(final @NotNull E event, final @Nullable T value) {
        if (value == null || processorsByTypeMap.isEmpty()) {
            return;
        }

        final var resultType = value.getClass();
        if (CompletionStage.class.isAssignableFrom(resultType)) {
            processPromise(event, (CompletionStage<?>) value);
            return;
        }

        final var resultProcessor = getResultProcessorByType(resultType);
    ...
    private <E extends Event, T> void processPromise(
            ...
            final @NotNull CompletionStage<T> promise) {
    ...
    private @Nullable ResultProcessor<?, ?> getResultProcessorByType(final @NotNull Class<?> resultType) {
        final var processor = processorsByTypeMap.get(resultType);
        if (processor != null) {
            return processor;
        }

        for (final var eventTypeToProcessor : processorsByTypeMap.entrySet()) {
